...
class dataUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            data_ocv= request.POST.get('Open_Circuit_Voltage_input')
            data_mh=request.POST.get('Maintenance_History_Neglected_input')
            data_mhm=request.POST.get('Maintenance_History_No_maintenance_input')
            data_mhr=request.POST.get('Maintenance_History_Regular_maintenance_input')
            data_acl=request.POST.get('Acid_Level_Check_Normal_input')
            #print (data)
            ...
            data = np.array([data_ocv,data_mh,data_mhm,data_mhr,data_acl])
            #sc = StandardScaler()
...
#data_for_prediction =obj.pca(np.array(data_for_prediction),y_test)
            ...
            return render(request, "succ_msg.html", {'data_ocv':data_ocv,'data_mh':data_mh,'data_mhm':data_mhm,'data_mhr':data_mhr,'data_acl':data_acl,
                                                        'out':out})
